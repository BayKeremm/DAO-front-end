...
const APIURL =
  "http://f587-54-87-32-29.ngrok.io/subgraphs/name/daodesigner/moloch-subgraph";
...
async function createProposal(details, sharesRequested, tributeOffered) {
  ...
  if (tributeOffered > 0) {
    ...
    if (balance._hex === "0x00") {
      console.log(
        "balance is",
        await token.connect(signer).balanceOf(signer.getAddress())
      );
    ...
    const allowance = await token
      .connect(signer)
      .allowance(signer.getAddress(), molochAddress);
    if (allowance._hex === "0x00") {
      console.log(
        "allowance to the moloch",
        await token
          .connect(signer)
          .allowance(signer.getAddress(), molochAddress)
      );
...
async function increaseAllowance() {
  ...
  const res = await token
    .connect(signer)
    .increaseAllowance(moloch.address, 5000);
  console.log(res);
...
const Home = () => {
  ...
  async function fetchProposals() {
    ...
    const tableProposals = response.data.moloch.proposals.map((e) => [
      ...
      e.details,
      <Tag
        color={e.sponsored ? "green" : "gray"}
        text={e.sponsored ? "yes" : "no"}
      />,
      <Tag
        color={e.processed ? "green" : "gray"}
        text={e.processed ? "yes" : "no"}
      />,
      <Tag
        color={e.didPass ? "green" : "red"}
        text={e.didPass ? "yes" : "no"}
      />,
    ]);
  ...
  const onSubmit = (data) => {
    console.log(data);
    createProposal(data.details, data.sharesRequested, data.tributeOffered);
  };
  console.log(errors);
            ...
            <div className="tabContent">
              ...
              <div>
                <form onSubmit={handleSubmit(onSubmit)}>
                  <input
                    type="number"
                    placeholder="sharesRequested"
                    {...register("sharesRequested", {})}
                  />
                  <input
                    type="number"
                    placeholder="tributeOffered"
                    {...register("tributeOffered", {})}
                  />
                  <input
                    type="text"
